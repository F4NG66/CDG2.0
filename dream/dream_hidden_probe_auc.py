...
OUT_JSON = "analysis_output/dream_hidden_probe_auc_torch.json"
OUT_CSV = "analysis_output/dream_hidden_probe_auc_torch.csv"
...
for contrast_name, (pos_g, neg_g) in contrasts.items():
    print("\nCONTRAST", contrast_name, flush=True)
    ...
    for scope, frac, layer in grid:
        ...
        print(
            ...
            f"n={len(y):3d} AUC={row['auc_mean']:.3f}±{row['auc_std']:.3f} "
            f"F1={row['f1_mean']:.3f}",
            flush=True,
        )
...
print("\n===== BEST BY CONTRAST =====", flush=True)

for c in contrasts:
    sub = [r for r in results if r["contrast"] == c]
    sub = sorted(sub, key=lambda r: r["auc_mean"], reverse=True)
