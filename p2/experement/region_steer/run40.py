#!/usr/bin/env python3
"""region_steer/run40.py — POWERED confirmation (n=40) generation, offline.
Balanced 20 strict-harm + 20 disclaimer B cases. Arms E/T/O/R/Rw (B), E/T/O (C, selectivity).
alpha in {21,42,84,126}. -v de-inject, block-16, identical RNG per case (seed=base+case#)."""
...
def main():
    ...
    ap.add_argument("--out", default=os.path.join(EXP, "region_steer/results/gen40.jsonl"))
    ...
    print(f"[run40] loading {MODEL_ID} bf16 ...", flush=True)
    ...
    print(f"[run40] dir {vmeta['kind']} L{LAYER} norm={vmeta['norm']:.4f}; cases={len(CASES)}", flush=True)
    ...
    print(f"[run40] {len(jobs)} jobs; {len(done)} done", flush=True)
    n = 0
    for grp, cid, arm, alpha, seed, scaffold, beh in jobs:
        ...
        n += 1
        if n % 25 == 0:
            print(f"[run40] {n} new (last {tid} len={row['metrics']['len']})", flush=True)
    fout.close(); runner.close()
    print(f"[run40] wrote {n} new rows -> {args.out}", flush=True)
