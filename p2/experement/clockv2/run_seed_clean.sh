#!/bin/bash
# clockv2/run_seed_clean.sh — close the last seed gap: the CLEAN arm at seeds 1 and 2.
#
# Why only clean: dija and benign_op already exist at seeds 1-2 (4 files x 38,400 rows,
# verified). The identity split dija-clean = (benign_op-clean) + (dija-benign_op) therefore
# needs only the clean arm to put BOTH the structure and content terms on the same seed
# footing as the headline. benign (arm 2) is NOT re-run: it is a reference contrast, not
# part of the arm-3 split.
#
# Launch detached:
#   cd /home/ore99/experement/clockv2
#   setsid nohup bash run_seed_clean.sh > results/seed_clean.log 2>&1 < /dev/null &
#
# Same guards as run_seed_rest.sh: flock (one instance ever — the earlier OOM was two
# capture processes on one 40GB MIG slice), sanitize-before-resume, headroom check, retry.
...
exec 9>"$LOCK"
if ! flock -n 9; then
  echo "[seed_clean] ANOTHER INSTANCE HOLDS THE LOCK ($LOCK) — refusing to start."
  exit 1
fi
...
for S in 1 2; do
  ...
  for ATTEMPT in 1 2 3 4 5; do
    ...
    if [ "${HAVE:-0}" -lt "$NEED_MIB" ]; then
      echo "$LOG_TAG seed=$S cond=$C attempt=$ATTEMPT: only ${HAVE}MiB free (<${NEED_MIB}); waiting 180s"
      sleep 180
      continue
    fi
    ...
    [ "$ROWS" -eq 38400 ] && break
    echo "$LOG_TAG incomplete ($ROWS/38400) — retrying after 60s"
    sleep 60
  done
  ROWS=$(wc -l < "$F" 2>/dev/null || echo 0)
